...
`class` を使って見た目を変えるところまで試すなら、[一つの箱にHTMLとCSSをつなぐ例](./05_ボックスとdisplay/01_一つの箱とdisplay.md#htmlとcssをつないで一つの箱を作る)へ進む。属性の全種類を先に読む必要はない。
...
- [05_ボックスとdisplay](./05_ボックスとdisplay/01_一つの箱とdisplay.md): 属性を持つHTML要素が画面上でどんな箱として扱われるか
- [04_HTMLの意味と構造](./04_HTMLの意味と構造/01_意味からHTML要素を選ぶ.md): `role` やARIAを含む意味構造
- [13_セレクタと構造依存](./13_セレクタと構造依存.md): `id` / `class` / 属性セレクタを使うときの構造依存
