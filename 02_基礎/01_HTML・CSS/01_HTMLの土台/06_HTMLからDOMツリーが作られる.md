...
DOMになるのはHTML要素だけではない。先ほどの`p`要素にある`こんにちは`という文字も、DOMではTextノードになる。
...
先ほどのDOMでは、`p`のElementノードが親で、`こんにちは`のTextノードが子になる。
...
ここでは`p`の子として、
`Text「こんにちは 」`と`strong Element`の2つがある。さらに`strong`の子として`Text「重要」`がある。
...
HTMLが入れ子になっていると、DOMも階層を持つ。もう一つ例を見る。
...
となる。[04_要素の入れ子と親子関係を読む](./04_要素の入れ子と親子関係を読む.md)で確認した要素同士の入れ子に加えて、DOMでは文字もTextノードとしてツリーに入る。
...
取得方法や変更方法は[DOMとイベント](../../02_JS/02_DOMとイベント.md)で扱う。
