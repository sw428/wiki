...
`class`をCSSの対象として使う最小例は[一つの箱を作る](../05_ボックスとdisplay/02_一つの箱を作る.md#htmlとcssを用意する)で確認する。クラス名の採用方針は[BEMとクラス命名](../../07_設計/CSS設計/01_BEMとクラス命名.md)、セレクタが実際に選ぶ要素は[セレクタが選ぶ要素を確認する](../14_セレクタと構造依存/01_セレクタが選ぶ要素を確認する.md)で扱う。
