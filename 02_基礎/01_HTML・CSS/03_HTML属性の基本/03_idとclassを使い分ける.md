...
`class`自体には、HTML標準の意味や動作は付かない。

CSSやJavaScriptなどから、制作側が共通の対象を選ぶために使う。

クラスを追加するかどうかは、スタイルや処理の対象を分類して示す必要があるかで判断する。
...
`id`は一つの要素を参照するための識別子、`class`は複数の要素にも使える分類として使い分ける。
