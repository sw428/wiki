...
クラスを増やすかどうかは、再利用するスタイルや処理の対象を明示する必要があるかで決める。命名の採用方針は[BEMとクラス命名](../../07_設計/CSS設計/01_BEMとクラス命名.md)、セレクタが実際に選ぶ要素は[セレクタと構造依存](../13_セレクタと構造依存/01_セレクタが選ぶ要素を確認する.md)で扱う。
