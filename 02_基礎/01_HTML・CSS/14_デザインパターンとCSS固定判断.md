...
このノートは、いま作るUIを1つ選んで引くためのパターン集。全パターンを順番に学ぶ必要はなく、関係のないUIのコードは後回しにしてよい。

- 先に `パターン早見表` で、どのUIに何を使うかを見る。
- 迷ったら、各パターンの `判断基準` で挙動理由を確認する。
- 仕様や細かい挙動は、最後の関連ノートや仕様リンクへ戻る。

最初の目安は、選んだUIについて「固定したいもの」と「可変に残すもの」を分け、担当する親・画像・文字・リンクなどを本文やAIと確認できること。迷ったら[最初に分けること](#最初に分けること)へ戻る。次は、そのUIで変わり得る画面幅・文字量・画像などを変え、[判断フロー](#判断フロー)の最後にある「読める・切れない・操作できる」を確認する。
...
- 詳しい使い分けは [mask-imageで見える範囲を作る](./10_画像と背景/02_mask-imageで見える範囲を作る.md) に戻る。
...
- `:nth-of-type()` を使う場合は、クラス順ではなくタグ種類の順番を見ている点に注意する。詳しくは [13_セレクタと構造依存](./13_セレクタと構造依存.md) に戻る。
...
`background-image` は要素の面積を作らないので、リンク側の `min-height`、`aspect-ratio`、`padding` などで押せる面を作る。画像自体が情報を持ち、HTMLの文字だけでは同じ内容を伝えられない場合は、背景ではなく `<img>` と代替テキストを使う。詳しい区別は [imgとbackground-imageを選ぶ](./10_画像と背景/01_imgとbackground-imageを選ぶ.md) を参照する。

### 判断基準

- 写真バナー: `aspect-ratio` + `object-fit: cover` で枠を固定しやすい。
- 文字入り画像、ロゴ、商品全体: `cover` で切らず、`width` + `height: auto` または `object-fit: contain` を先に検討する。
- ロゴ素材の比率とカンプの測定値がわずかに違う場合は、素材比率を保った表示を先に確認する。厳密な寸法一致が必要なら、実装で変形・切り抜きせず、素材データまたはデザイン側の修正範囲として確認する。
...
iframeは外部の文書やプレイヤーをページ内へ埋め込む要素。srcには埋め込み対象URL、titleには何の埋め込みか分かる名前を付ける。YouTubeなら通常の視聴URLではなく埋め込み用URLを使う。

allowfullscreenは全画面表示の許可、allowは機能許可の列挙。媒体の要件に合わせて必要な機能を選ぶ。そのうえで、以下の外側の比率枠と中の表示を組む。
...
リンク側へpadding・min-heightを持たせると、文字の周辺にも操作範囲を広げられる。text-alignは主にインライン内容の横方向の配置であり、縦中央やクリック範囲を作る指定ではない。

内容変化に耐えられるpaddingとmin-heightを基準にし、均等幅が必要なら幅指定を追加する。区切り線、ナビ全体の余白、リンクの操作範囲は別々の責務として扱う。

## 装飾アイコンを疑似要素にするか実要素にするか

矢印や三角形が装飾なのか、個別に扱う部品なのかを先に分ける。単純な装飾は疑似要素、JSから直接操作するアイコンはspanなどの実要素を基本候補にする。
...
文字生成は、装飾として余計に読み上げられないか確認する。単純な形ならCSSの線やborderによる三角形も候補。SVGなどの素材を使う場合は、次のように実要素で扱える。
...
画像が装飾ならaltを空にする。画像自体の寸法を指定する場合や、役割を識別する必要がある場合にクラスを付ける。画像・SVGを使うことと、必ずラッパーを増やすことは別に判断する。

状態連動は実要素が必須というCSS仕様ではない。親の属性やクラスに応じて疑似要素の見た目を変えることもできる。既存の「状態と同期する部品は実要素で管理する」という実装方針と、技術上可能なことを分ける。

アイコンだけのボタンでも操作名は必要。[開閉UIの最小例](../02_JS/03_UI実装パターン.md#最小テンプレアイコンだけの開閉ボタン)で、見た目と名前・状態をそろえる。
...
- [10\_画像と背景](./10_画像と背景/00_画像と背景整理マップ.md): 画像をHTMLで置くか背景で置くか、`mask-image` で形抜きするか
- [09\_レイアウト](./09_レイアウト/00_レイアウト整理マップ.md): 親で並びと余白を管理する判断
- [11\_メディア設計](./11_メディア設計/00_メディア設計整理マップ.md): 画像・動画の比率、切り抜き、CLS対策
- [12\_表示制御](./12_表示制御/00_表示制御整理マップ.md): SP/PCで存在を切る判断
- [13\_セレクタと構造依存](./13_セレクタと構造依存.md): `+` / `:nth-child()` / `:nth-of-type()` など、構造依存セレクタの判断
