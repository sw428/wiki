...
`display`とboxを同じ言葉として扱わず、指定と結果として分ける。詳しい関係は[box treeとボックスモデル](../05_ボックスとdisplay/02_box-treeとボックスモデル.md)で確認できる。
...
単純な文字記号なら疑似要素で表せる。SVG画像を使う場合や、独立した意味・細かな寸法調整が必要な場合は実要素も検討する。具体的な判断は[CTAとナビリンクの操作範囲](../15_デザインパターンとCSS設計判断/07_CTAとナビリンクの操作範囲を作る.md)と[装飾アイコンとラベル](../15_デザインパターンとCSS設計判断/09_装飾アイコンとラベルを小さな部品として扱う.md)へ進む。
