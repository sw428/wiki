...
生成された箱の関係を、ここではbox treeと呼ぶ。表示までの処理全体は[ブラウザの基本フロー](../07_ブラウザ挙動と表示のズレ/01_ブラウザが表示を作る基本フロー.md#基本フロー)で確認できる。
...
画像や動画で `aspect-ratio` を使う実装は[比率の枠へ画像を収める](../11_メディア設計/01_レスポンシブ画像設計/03_同一比率フレーム型.md)で扱う。
