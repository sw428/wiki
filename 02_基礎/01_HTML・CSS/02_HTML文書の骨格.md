...
まず [最低限の基本形](#最低限の基本形)を見本にし、`head` に置く情報と `body` に置く本文を分ける。タグ名そのものが読みにくい場合だけ、[HTMLの土台](./01_HTMLの土台.md#タグと要素は同じではない)の具体例へ戻る。

- **最初に確かめること:** `charset`、`title`、`viewport` の役割と、本文を書く場所、CSSファイルの読み込み先。
- **必要になってから読むこと:** JSを読み込むときの `defer` と配置の違い。`head` に置ける情報の全種類や `base` は、最初の文書を作るために全部覚える必要はない。
- **次へ進む目安:** 見本やAIを使いながら、ページタイトル・本文・CSSパスのどこを変えるかを選び、ブラウザで意図した表示になったか確認できること。JSを使う場合は、その読み込み位置も本文と照合する。

そこまでを足場に [HTML属性の基本](./03_HTML属性の基本.md)へ進める。文字化け、CSSが当たらない、JSが対象要素を取れないといった疑問が出たら、このページの該当する読み込みの説明へ戻る。仕組みの詳細は、その疑問と結び付けて読み足す。
...
同じカスケード条件なら、後に位置する宣言が勝つ。これはダウンロード完了の速さではなく、HTMLからリンクした順番で決まる。詳しい確認方法は [CSSカスケードとDevToolsの見方](./06_CSSカスケードとDevToolsの見方/01_DevToolsで適用されたCSSを確認する.md) で扱う。
...
- [05_ボックスとdisplay](./05_ボックスとdisplay/01_一つの箱とdisplay.md): HTML要素が画面上でどんな箱として扱われるか
- [04_HTMLの意味と構造](./04_HTMLの意味と構造/01_意味からHTML要素を選ぶ.md): 本文内の見出し・section・footerなどの意味構造
