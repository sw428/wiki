...
まず画像1枚を対象に、「全部見せたいか、決めた枠へ収めたいか」を分ける。[実務での基本形](#実務での基本形)と[まず2択で決める](#まず2択で決める認知負荷を下げる)を入口にし、円形画像、地図のピン、画像候補の切り替えなどは、扱う場面で読み足せばよい。

最初の目安は、本文やAIを使いながら、HTMLの寸法属性とCSSの表示幅を見分け、幅を変えた時に画像全体が見えるか・切れるかを確認できること。枠の大きさと画像の見え方が混ざったら、[外・箱・中身で分解する](#外箱中身で分解する)へ戻る。

必要になったところから、次の認識を増やす。

- 画像に文字を重ねたい: [箱だけで重ねる練習](#箱の中央に文字を重ねる)から始め、[画像を使う基本形](#画像を使って重ねる)へ進む。

- 枠へ収めたい: [画像自身と親のどちらに比率を置くか](#画像自身と親のどちらにaspect-ratioを置くか)で、枠を担当する要素を選ぶ。
- PC/SPで違う画像を使いたい: [画像候補と表示する要素の分担](#pictureでpcsp画像の比率が違う場合)を読み足す。
- 画像のピクセル寸法と表示サイズが混ざった: [寸法・解像度・比率](#寸法解像度比率を分ける)へ戻り、必要なら[2倍書き出し画像の扱い](#2倍書き出し画像の扱い)へ進む。
...
| 見ているもの | 例 | 意味 |
|---|---|---|
| 画像データのピクセル寸法 | `1600 × 900px` | ファイルが持つ画素数。PPIや画面上の表示寸法とは別の情報 |
...
「解像度」は文脈によって画素数、PPI、画面の表示密度などを指し得る。ここでは曖昧にまとめず、ファイルの幅と高さを指す場合は「画像データのピクセル寸法」と書く。

HTMLの `width` / `height` は [dimension attributes（寸法属性）](https://html.spec.whatwg.org/dev/embedded-content-other.html#dimension-attributes) である。2つの寸法からブラウザは比率も得られるが、CSSの `aspect-ratio` と同じ意味ではない。
...
たとえば `width="800" height="450"` の16:9画像へ `width: 100%; height: auto;` を指定し、親の使用幅が600pxなら、ほかの制約がない場合の表示寸法は `600 × 337.5px` になる。HTML属性が800 × 450という情報を持つことと、最終表示がその寸法へ固定されることは別である。

### DevToolsのAttributes Styleと最終表示を分ける

`<img width="800" height="450">` のような寸法属性は、DevToolsのStylesで `img[Attributes Style]` など、属性由来の表示として見えることがある。これはCSSファイルへ同じ宣言を書いたという意味ではない。

- CSSで表示幅・高さが上書きされると、画面上の最終寸法はCSSとレイアウト計算で決まる。
- 属性の幅・高さが最終表示寸法として使われなくても、2値から得られる固有比率は読み込み前の領域予約に利用され得る。
- Stylesの打ち消し線だけを見て「属性が完全に無意味になった」と判断せず、Computedの最終寸法と、比率予約の有無を分けて確認する。
...
基準幅375pxのカンプで画像領域が `375 × 400` なら、`height: 400px` と `aspect-ratio: 375 / 400` はその幅で同じ結果になる。1枚のカンプだけでは、固定高と比率維持のどちらがデザイン意図かは判定できない。

| 優先するもの | 枠の決め方 |
|---|---|
| 画像の構図を保ち、幅に応じて高さも変えてよい | `aspect-ratio` または画像の自然比率 |
| ヒーロー領域の高さや次の内容の開始位置を保ち、切り抜きを許容する | `height` / `min-height` と `object-fit: cover` |

別幅のカンプから変化を読むか、デザイナーへ確認して決める。確認できない場合は、どちらを優先した暫定案かを明記し、基準幅以外でも構図と占有面積を確認する。`picture` による画像候補の切り替えと、CSSによる表示領域の決定は別の判断である。

実装例でも両方式が使われている。[Shopify DawnのImage banner](https://github.com/Shopify/dawn/blob/main/sections/image-banner.liquid)は画像比率へ合わせる設定と高さプリセットを持ち、[WordPressのCoverブロック](https://wordpress.org/documentation/article/cover-block/)はアスペクト比と最小高さの両方を選べる。これらは選択肢が両方ある根拠にはなるが、個別カンプの意図を確定する根拠にはしない。
...
まず色付きの箱で2つの指定を切り分け、その後で画像を使う。箱を広げる指定と中央寄せの違いを確認できていれば、[画像を使って重ねる](#画像を使って重ねる)から読める。
...
画像へ重ねたい場合は[画像を使って重ねる](#画像を使って重ねる)へ進む。画像が通常の配置に残って外側の高さを作るので、この練習用の固定高さはそのまま持ち込まない。

imgだけ広がらない場合は[条件の違い](../../03_詳細/01_HTML・CSS/01_insetで広がる条件を調べる.md)を見る。

### 画像を使って重ねる

色付きの箱で試した重ね方を、画像へ応用する。ここでは通常の配置に残る画像が外側の高さを作る。
...
`absolute`で配置する要素の`top`・`left`は、基準の上端・左端からの距離を決める。`inset: 0`は上下左右の距離をまとめて0にする指定。幅・高さが`auto`などの条件を満たすと、両端の位置からサイズが決まり、結果として箱が広がる。

例えば`<p class="banner__text">お知らせ <span>新着</span></p>`なら、`.banner__text`に書いた`absolute`と`inset`が操作するのはpの箱。中の文字まで引き伸ばす指定ではない。

`grid`を付ける前の通常の配置では、pが広がっても、中のspanはインラインの文字サイズのまま。pの`position`や`inset`がspanへ自動で引き継がれることもない。

指定の役割が混ざったら、[箱の中央に文字を重ねる練習](#箱の中央に文字を重ねる)へ戻り、中央寄せの2行を外して確認する。

`inset: 0`で全面になるには、幅・高さ・余白などの条件がある。imgのサイズ計算も異なるため、広がらない場合は[条件の違い](../../03_詳細/01_HTML・CSS/01_insetで広がる条件を調べる.md)を確認する。
...
1. 問題になっているのは、画像データのピクセル寸法、HTMLの寸法属性、CSS計算後の表示寸法、アスペクト比のどれか
2. 先に比率を予約できるか（`width` / `height` 属性 or `aspect-ratio`）
3. トリミング許容か（`cover`）/ 余白許容か（`contain`）
4. これは「外」「箱」「中身」のどれを調整しているか
5. 背景装飾の問題なら 06 側で切り分ける

## 一言でいうと

画像データのピクセル寸法、HTMLの寸法属性、CSS計算後の表示寸法、アスペクト比を分ける。画像や動画は、外側の配置、比率を持つ箱、中身の収め方を分離し、CLS対策は比率予約から始める。
