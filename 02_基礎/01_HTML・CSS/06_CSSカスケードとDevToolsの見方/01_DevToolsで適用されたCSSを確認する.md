...
打ち消し線だけでは、「詳細度で負けた」とは限らない。まず、選んだ要素、プロパティ、出典ファイル、最終値を確認する。どの宣言が勝ったかという理由が必要になったら、[優先条件が同じときの記述順](./02_優先条件が同じときの記述順.md)へ進む。
...
競合があれば[優先条件が同じときの記述順](./02_優先条件が同じときの記述順.md)と[セレクタの詳細度を比べる](./03_セレクタの詳細度を比べる.md)、`!important`やレイヤーまで関係する場合は[カスケード全体の優先順位](./04_カスケード全体の優先順位.md)を使う。
