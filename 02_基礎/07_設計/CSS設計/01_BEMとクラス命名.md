...
BEMはHTMLに付けるクラス名の決め方。入れ子はCSSやSCSSをまとめて書く方法。この2つは併用できる。

例えば、次のSCSSは`.card__title`というクラスの文字を大きくする。
...
一方、`.card { .title { ... } }`という入れ子は、`.card`の中にある`.title`を選ぶ。`.card__title`という名前を作る処理とは違う。

`&__title`のような文字列の連結はSassの機能。通常のCSSの入れ子では使えないので、CSSへ直接書くなら`.card__title`と書く。[Sassの接尾辞追加](https://sass-lang.com/documentation/style-rules/parent-selector/#adding-suffixes)と[CSSの入れ子セレクタ](https://www.w3.org/TR/css-nesting-1/#nest-selector)を参照。

管理しやすさを見るときは、見た目のまとまりに加えて「HTMLの入れ方を変えると、どのセレクタが効かなくなるか」を見る。子孫セレクタを深く連ねるほど、その祖先関係が必要になる。
...
すべてのdivや画像へ、BEMの形を埋めるためだけにクラスを付けない。CSS・JSから識別する必要、部品内での役割、再利用の範囲を先に確認する。

画像にクラスを付けるのは、画像自体へ幅・比率・object-fit・角丸などを指定する場合、同じ親の複数画像を区別する場合、JSから識別する場合、再利用部品内の役割を明示する場合が候補になる。親の配置や共通のimgルールだけで足りるなら、画像専用クラスがなくてもよい。

名前はDOMの入れ子を写すのではなく、担当する役割から選ぶ。`box / wrap / content` だけでは役割が伝わらない場合は具体化する。同じ外見でも責務や再利用範囲が異なるなら、別クラスを検討する。

HTML要素自体の選択は[HTMLの意味と構造](../../01_HTML・CSS/04_HTMLの意味と構造/01_意味からHTML要素を選ぶ.md)、画像アイコンの囲み方は[装飾アイコンとラベル](../../01_HTML・CSS/14_デザインパターンとCSS固定判断/09_装飾アイコンとラベルを小さな部品として扱う.md)へ分担する。
