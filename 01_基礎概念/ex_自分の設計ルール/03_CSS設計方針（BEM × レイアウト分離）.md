# 03_CSS設計方針（BEM × レイアウト分離）

## 位置づけ（設計ルール）

このファイルは、検証と制作ログから抽出したCSS設計方針を、自分の採用ルールとしてまとめる本体。
...
- `__` は1段まで（[DOM](../../03_参照元/DOM.md) の深さと命名は一致させない）
...
typography用クラスは、同じ文字スタイルを複数箇所・複数ページで使い回すときに作る。
