...
- 画面に出る対象は、[DOM](../../04_参照元/DOM.md) と [CSSOM](../../04_参照元/CSSOM.md) を合成した結果（レンダーツリー）で決まる（[状態変化](../../04_参照元/状態変化.md)）。
- `display` は「どの種類のボックスを生成するか」に関与する。
- 文字は [テキストノード](../../04_参照元/テキストノード.md) として扱われ、指定反映は [CSS適用境界](../../04_参照元/CSS適用境界.md) を分けて見る。
...
- `inline`: 横幅は内容依存。`width`/`height` は通常効かない（[置換要素](../../04_参照元/置換要素.md) を除く）。
...
この例で `.child` のDOM上の親は `.parent` だが、絶対配置の包含ブロックを作るのは、最も近い位置指定された祖先 `.outer` である。そのため、`.child` の幅は `.outer` の800pxを基準に400pxとなる。
