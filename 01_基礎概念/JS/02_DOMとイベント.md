...
- [DOM](../../03_雑記/知識辞書（試運用）/02_単語カード/DOM.md)
- 要素の取得
- 属性とプロパティ
- class操作
- 文字やHTMLの変更
...
[DOM](../../03_雑記/知識辞書（試運用）/02_単語カード/DOM.md) は、HTMLをJavaScriptから触れるようにしたオブジェクト構造。
...
JSコードが要素より先に実行されると、要素取得が `null` になることがある。
...
`defer` があると、DOM構築後にJSコードが実行されるため、
...
ここは「DOMとイベント」の延長として、次回以降に詳細を追記するための枠。
いまは見出しだけ固定し、本文は実践ケースの蓄積後に追加する。

### イベント委譲

- 親要素に1つイベントを付け、子要素の操作を拾う基本形を追加予定。

### form / input / submit の読み方

- `input` / `change` / `submit` の使い分けを、実装順で整理して追加予定。

### イベント対象の見分け方

- `event.target` / `currentTarget` / `closest()` の判断順を追記予定。
