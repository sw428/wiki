...
このページは、身近な表示から仕組みへ戻る順でも読める。最初は、背景の付いた要素を一つ選び、その要素と親、`display`、幅・高さ、内側と外側の余白を確認する。

1. [例](#例)の `div` と `span` で、背景の広がり方を比べる。
2. [ボックスモデルと表示形式の区別](#ボックスとボックスモデルの区別)で、表示形式・寸法・余白のどれを見ているか分ける。
3. 広さの理由が分からなければ、[見えている広さを「余白」と決めつけない](#見えている広さを余白と決めつけない)と[DevToolsのボックスモデル図](#devtoolsのボックスモデル図で見ている値)へ戻る。

今は、`display` を変えることと、`box-sizing` や余白を変えることの違いを、対象の箱で確認する。CSSの計算段階や匿名ボックスの種類を一度に覚える必要はない。指定から表示までを詳しく追いたくなった時に、[CSSルール・計算値・box tree・Paint の関係](#cssルール計算値box-treepaint-の関係)を読む。

**次へ進む目安**は、WikiやAIを見ながらでも、一つの要素について「どの箱の、どの寸法や余白を変えたか」を説明し、実寸と表示を確かめられること。割合指定で予想と違う大きさになったら、[実装時の確認順](#実装時の確認順)から、プロパティごとの `%` の参照先へ確認範囲を広げる。親の余白を消しても別の余白が残る時は、[リセットCSS](#リセットcssは何を消したかで読む)へ進む。

## ルール

- [DOM](../../05_参照/DOM.md) などの要素ツリーに、カスケードと継承で各CSSプロパティの計算値が割り当てられ、`display` などに従ってCSSのbox treeが作られる。その後、LayoutとPaintを経て画面へ描画される（[状態変化](../../05_参照/状態変化.md)）。
- `display` は「どの種類のボックスを生成するか」に関与する。
- 文字は [テキストノード](../../05_参照/テキストノード.md) として扱われ、指定反映は [CSS適用境界](../../05_参照/CSS適用境界.md) を分けて見る。
...
CSSは解析され、スタイルシートやCSSルールとして扱われる。[CSSOM](../../05_参照/CSSOM.md) は、そのスタイルシートやルールをオブジェクトとして参照・操作するための仕組みである。
...
- `inline`: 横幅は内容依存。`width`/`height` は通常効かない（[置換要素](../../05_参照/置換要素.md) を除く）。
...
中央の `618 × 130` と周囲の数値は、CSSソースに書いた宣言をそのまま表示したものではない。カスケードや継承を解決した後、包含ブロック、内容、`box-sizing` などを使ってLayoutで求めた `content / padding / border / margin` の寸法を、CSSピクセルで確認するための表示である。

```txt
CSSの宣言
-> カスケード・継承・computed value
-> Layoutで使用寸法が決まる
-> DevToolsのボックスモデル図で確認する
-> Paint・実際の画面表示
```

全体の描画工程から見れば、この図の数値もPaint前の中間にある。ただし、CSSの指定値からまだ計算されていない仮の値ではなく、**レイアウト後の使用値（used value）に近い結果**として読む。レイアウトを調べる場面では、現在の要素が占めている寸法を確認する値である。
...
8. [詳細度・上書き順](./15_CSSカスケードとDevToolsの見方.md)（同じカスケード条件なら後勝ち）
