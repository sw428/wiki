...
このページは、最初から全項目を通読する代わりに、いま作る部分の役割から読む範囲を選べる。まず [HTMLの骨組みを役割で切る](#htmlの骨組みを役割で切る)の比較を見て、見た目用の箱と文書上の役割を分ける。用語が曖昧なら [HTMLの土台](./01_HTMLの土台.md#タグと要素は同じではない)の短い例へ戻る。

| いま決めたいこと | 次に読む説明 |
|---|---|
| ページやカードの骨組みを作る | [制作での判断表](#制作での判断表)から候補を探し、その要素の節で文脈と例外を見る。見出しは大きさと階層を分ける説明まで読む |
| 押せる部品を作る | [リンクとボタン](#リンクとボタンは押した結果で分ける)で結果を分け、開閉する部品なら [A11y属性](#開閉uiでのa11y属性の整理)も合わせる |
| 文字や図を画像へまとめるか迷う | [文字と意味構造を画像へまとめる前に](#文字と意味構造を画像へまとめる前に)で、意味・操作・更新のどこを残すかを確認する |

DOM / CSSOM / box tree / AOM の用語整理や、まだ使わない要素の細かな分岐は後回しにできる。最初の目安は、ページの一部分について、本文やAIの助けを使い「この要素を選ぶ理由」と「見た目はCSSで決める部分」を分けて確認できること。迷う条件を言葉にできたら、その条件に対応する節を読み足しながら実装へ進んでよい。

再確認では、図版、住所、ナビ、日時など、対象と同じ本文見出しへ戻る。[初期認識メモ](#初期認識メモ確度)に残る強すぎた判断は、現在の本文にある理由・条件と対にして読む。
...
- [DOM](../../05_参照/DOM.md) には要素ノードと [テキストノード](../../05_参照/テキストノード.md) があり、指定反映は [CSS適用境界](../../05_参照/CSS適用境界.md) を分けて見る。
