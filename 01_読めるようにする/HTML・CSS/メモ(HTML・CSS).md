...
- 反映先:
  ...
  - [03_HTML属性の基本](./03_HTML属性の基本.md)
  - [04_ボックスとdisplay](./04_ボックスとdisplay.md)
  - [05_インラインと行の仕組み](./05_インラインと行の仕組み.md)
  - [06_画像と背景](./06_画像と背景.md)
  - [07_レイアウト](./07_レイアウト.md)
  - [08_ブラウザ挙動とズレ](./08_ブラウザ挙動とズレ.md)
  - [09_HTMLの意味と構造](./09_HTMLの意味と構造.md)
  - [10_メディア設計（img・video・object-fit・aspect-ratio）](./10_メディア設計（img・video・object-fit・aspect-ratio）.md)
  - [11_表示制御（sp-only／pc-only／revert設計）](./11_表示制御（sp-only／pc-only／revert設計）.md)
  - [12_セレクタと構造依存](./12_セレクタと構造依存.md)
...
- 実務判断で迷う内容は、必要に応じて `04_検証` 側へ運用ルールとして降ろす
...
- [01_CSS設計整理マップ](../設計/CSS設計/00_CSS設計整理マップ.md)
  - コンポーネント核とページ側上書きの境界
  - 上書き可否の判断
  - サイズ差分をModifierで扱う方針
- [05_インラインと行の仕組み](./05_インラインと行の仕組み.md)
...
- 要望: `01_CSS設計整理マップ` をポートフォリオ側へ移管し、HTML・CSS番号を1つ繰り上げ
- 反映:
  - `01_読めるようにする/HTML・CSS`
    - `02〜09` を `01〜08` へリネーム
    - 基礎概念内リンクを新番号へ更新
  - `01_読めるようにする/設計/CSS設計`
    - `00_CSS設計整理マップ.md` を入口として配置
...
- 反映:
  - [01_ケース_Blogカード画像_レンダリング挙動](../../04_検証/portfolio/01/01_ケース_Blogカード画像_レンダリング挙動.md)
...
- 反映:
  - [00_HTML・CSS整理マップ](./00_HTML・CSS整理マップ.md)
    - `01〜08`（基礎概念）と `04_検証`（個別検証）の運用モード分離を明記
    - ケース検証運用ファイルへの導線追加
  - [00_ケース検証運用](../../04_検証/00_ケース検証運用.md)
    - 命名ルール、置き場、記録テンプレを追加

## 追記ログ（2026-05-03-3）

- 要望: `04_検証` フォルダ内を番号運用に統一（`00`=運用、`01`=今回ケース）
- 反映:
  - `04_検証/00_ケース検証運用.md`
    - ケース分析 -> 知識側（01〜08）へ戻す手順を明記
  - `04_検証/portfolio/01/01_ケース_Blogカード画像_レンダリング挙動.md`
    - 今回ケースを個別ファイルとして固定
...
- 反映:
  - [00_ケース検証運用](../../04_検証/00_ケース検証運用.md)
    - ケース検証を `04_検証` に集約し、案件別フォルダで受ける運用へ更新
...
- 反映:
  - [02_ケース_Blog一覧_SP基準設計と余白責務](../../04_検証/portfolio/01/02_ケース_Blog一覧_SP基準設計と余白責務.md)
    - 認知アプローチ（分解手順）と結果を整理
  - [07_レイアウト](./07_レイアウト.md)
...
- 受け取り元: cj-pro 01の制作ログ（2カラム / CTA / Voiceカード / Flex言語化）
- 反映:
  - [01_ケース_2カラムとCTA・Flex配置判断](../../04_検証/portfolio/02/01_ケース_2カラムとCTA・Flex配置判断.md)
...
- 就職・給与・支援制度の話は対象外にし、デザインパターンからCSS責務へ落とす基礎概念として整理
- 反映:
  - [13_デザインパターンとCSS固定判断](./13_デザインパターンとCSS固定判断.md)
