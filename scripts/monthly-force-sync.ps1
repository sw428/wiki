...
function Get-AutoSummary {
    param([string]$Fallback = "monthly sync")
    ...
    foreach ($line in $statusLines) {
        ...
        # Rename lines look like: old/path -> new/path
        if ($path -match " -> ") {
            $path = ($path -split " -> ")[-1]
        }
...
function Read-State {
    ...
    try {
        ...
        Write-Warning "[monthly-sync] failed to parse state file; continuing without state"
        return $null
    }
}
...
$stateFile = Join-Path -Path $gitDir -ChildPath "monthly-force-sync-state.json"
$state = Read-State -Path $stateFile

$targetBranch = Get-TargetBranch -RequestedBranch $Branch
$timestamp = Get-Date -Format "yyyy-MM-dd_HHmmss"
$targetPeriod = if ($CatchUp) { (Get-Date).AddMonths(-1).ToString("yyyy-MM") } else { (Get-Date).ToString("yyyy-MM") }
$summaryText = if ([string]::IsNullOrWhiteSpace($Summary)) { Get-AutoSummary } else { $Summary.Trim() }
$commitMessage = "$targetPeriod / $summaryText"
$backupBranch = "backup/$timestamp"
$modeText = if ($CatchUp) { "catch-up" } else { "monthly" }

if ($CatchUp -and $null -ne $state -and $state.last_synced_period -eq $targetPeriod) {
    Write-Host "[monthly-sync] catch-up skipped; period $targetPeriod already synced"
    return
}

Write-Host "[monthly-sync] mode: $modeText"
Write-Host "[monthly-sync] branch: $targetBranch"
Write-Host "[monthly-sync] remote: $remoteUrl"
Write-Host "[monthly-sync] commit: $commitMessage"
Write-Host "[monthly-sync] backup: $backupBranch"
...
if ($hasChanges) {
    ...
    Write-Host "[monthly-sync] no file changes; skip commit"
}

# Safety net before force-style push.
& git push $Remote "HEAD:refs/heads/$backupBranch"
if ($LASTEXITCODE -ne 0) {
    throw "git backup push failed with exit code $LASTEXITCODE"
}

# Force update main branch safely (lease check prevents blind overwrite).
& git push --force-with-lease $Remote "HEAD:refs/heads/$targetBranch"
if ($LASTEXITCODE -ne 0) {
    throw "git force-with-lease push failed with exit code $LASTEXITCODE"
}

$head = Get-GitText -Args @("rev-parse", "HEAD")
Write-State -Path $stateFile -Data @{
    last_synced_period = $targetPeriod
    last_mode = $modeText
